    ...
    public class InvoiceDocument : PdfDocument<InvoiceSource>
    {
        ...
        protected void ComposeContent(IContainer container)
        {
            container.PaddingVertical(20).Column(column =>
            {
                ...
                column.Item().Element(ComposePurschaseOrder);
        ...
        protected void ComposePurschaseOrder(IContainer container)
        {
            container.ShowOnce().DefaultTextStyle(tStyle => tStyle.SemiBold()).Row(row =>
            {
                row.RelativeItem().Column(column =>
                {
                    column.Item().Text(t => ComposeField(t, Source, x => x.POOrder, delimiter: ":"));
                });
        ...
        protected void ComposeSalesQuotationHeader(IContainer container)
        {
            container.ShowOnce().DefaultTextStyle(tStyle => tStyle.SemiBold()).Row(row =>
            {
                row.RelativeItem().Column(column =>
                {
                    column.Item().Text(t => ComposeField(t, Source, x =>x.SalesQuotationNumberText));
                    column.Item().Hyperlink(Source.StoreUrl).Text(Source.StoreUrl);
                    column.Item().Text(t =>
                        ComposeField(t,
                            Source,
                ...
                var logoContainer = row.ConstantItem(65).Height(65).BorderBottom(1).BorderColor(Colors.Grey.Lighten2);
        ...
        public override void Compose(IDocumentContainer container)
        {
            container
                .Page(page =>
                {
                    ...
                    page.Header().Element(ComposeSalesQuotationHeader);
